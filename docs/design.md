...
> **本書のステータス（2026-06-24 更新）:** これは 2026-03 の設計記録であり、随所に Python 前提の記述（言語・`pipeline.py`・`uv`/`ruff`/`mypy`・ディレクトリ構成・コードサンプル等）が残る。**現在の正は ADR-006（前提再検証）/ ADR-007（Rust 移行）/ README** とし、本書は背景・設計意図の参照元として維持する。各節の個別判断の更新は対応する ADR を参照。
...
[Smoke Test]         ← Manager: docker compose up → curl /health → smoke test script → finally: docker compose down
       ↓  ← fail → Agent A に差し戻し（最大3回、超過→エスカレーション）
...
```python
def run_agent(prompt: str, timeout: int = 300, **kwargs) -> str:
...
| レイヤー | 脅威 | 対策 |
|----------|------|------|
| 実行環境 | ホスト OS の破壊、クレデンシャル窃取・外部送信 | Claude Code sandbox（filesystem + network 隔離） |
| GitHub 権限 | token 権限内での暴走（main 破壊、大量 PR 等） | Fine-grained PAT（権限最小化）+ Branch Protection |
...
> **更新（2026-06）:** 実装言語は Rust に変更。本節の Python 採用判断は **ADR-007 が supersede**。再評価の背景（目的変更・課金・ネイティブ primitive）は ADR-006 参照。
...
> **更新（2026-06, ADR-007）:** 言語は Rust。本節の Python 前提（`dataclass`/`Enum`/`match-case`、200〜300 行目標）は Rust に読み替える（行数目標は 400〜600 行、`enum` + `match` の網羅性チェックを活用）。
...
- AGENTS.md, docs/, pyproject.toml（uv）, CI を整備
...
### 6.6 セキュリティ設計について（2026-03-16 セッション）

> **更新（2026-06, ADR-006）:** 本節の課金前提（Agent SDK は Max 定額不可、`claude -p` は定額で動く）は 2026-06-15 告知（現在 pause 中）の変更で揺らいでいる。詳細は ADR-006「前提変更 2」参照。

- 脅威モデルを 3 層（ホスト保護 / クレデンシャル窃取 / 権限内暴走）に分離して検討した。sandbox も container も microVM も、3 番目の脅威（渡した token の権限内での暴走）には無力であり、ここは branch protection + token スコープで守る領域。
- Docker Sandbox は Linux（DGX Spark）では microVM ではなくコンテナベースの隔離になること、Max プランの OAuth 認証との相性問題があることから不採用。
- 「container + YOLO モード」より「sandbox on host」の方が、AI エージェントの脅威モデルに対して的確に機能する。一見パラドキシカルだが、sandbox はツール実行単位で filesystem + network を制限するのに対し、container + YOLO はコンテナ内での行動を一切制限しない。
- Claude Code の sandbox はネットワークを unix domain socket 経由のプロキシで制御するため、outbound（exfiltration）も inbound も構造的に遮断される。
- Claude Agent SDK（Python）はフェーズごとの権限制御が自然にできるが、Max プランの定額利用ができない（API キー課金が必要）ため現時点では不採用。パイプラインが価値を証明した後に再評価する。
- 将来の hardening として、Claude Code on the web が採用している「git プロキシパターン」（token を直接渡さず、プロキシがブランチ名や操作種別を検証してから代理実行する）が参考になる。
...
- [ ] sandbox 設定の具体的なチューニング（allowedDomains の精査、denyRead の範囲）
- [ ] Fine-grained PAT の作成と権限設定
- [ ] Branch Protection Rules の設定
- [ ] Git プロキシパターンの実装判断（Phase 1 以降）
