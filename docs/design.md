...
- **ノールックマージツールではない。** 最終マージの判断は人間が行う。
...
1. **マネージャーは LLM ではなく決定論的プログラムとして書く。** LLM にステージ遷移の判断を委ねるとブレるリスクがある。pass/fail 判定、リトライカウンタ、エスカレーション判断はすべてコードで制御する。
...
[実装]               ← Agent A: plan.md に従って実装
       ↓
[テスト実行]         ← Manager: make test, lint, type check
...
├── Makefile               # test, lint, smoke-test 等のタスクランナー
├── pyproject.toml
└── tests/
```
