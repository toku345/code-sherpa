...
- **日付:** 2026-03-16
- **関連:** ADR-002 (shell-kick), セクション 2.4（エージェント接続方式）
...
2. **クレデンシャルの窃取** — prompt injection で `~/.ssh/` や `~/.claude/.credentials.json` を読み取り、外部に送信する
...
Docker 公式の sandbox 機能。macOS/Windows では microVM、Linux ではコンテナベースの隔離を提供する。
