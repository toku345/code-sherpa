...
> **更新（2026-06, ADR-006）:** 2026-06-15 告知（現在 pause 中）の課金変更により、`claude -p` も Agent SDK と同じく定額枠から分離される方向。「SDK は課金・shell-kick は定額」という本選択肢の前提は揺らいでおり、SDK vs shell-kick は今後「課金」ではなく「機能と複雑さ」で判断する。詳細は ADR-006「前提変更 2」参照。
