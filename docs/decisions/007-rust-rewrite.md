...
1. **配布の摩擦:** Python はインタプリタ / venv / `uv` を前提とし、自分が日常的に使うツールとして配布・起動が煩雑。
...
- **依存は最小限**（design.md §3.2 の「薄いスクリプト」精神を Rust に引き継ぐ）:
  ...
  | `clap` | CLI 引数（`code-sherpa <issue_number> --repo <owner/repo>`） | `std::env::args` で代替も可。idiomatic な `clap` を採用 |
