...
- `gh` CLI / `git push/pull`: `dangerouslyDisableSandbox: true` で実行可能
- `codex` CLI: macOS システム設定へのアクセスがブロックされ起動不可。ユーザーに手動実行を依頼する

## PR レビュー対応

```bash
gh pr view --json comments,reviews                          # レビューコメント取得
gh api repos/{owner}/{repo}/pulls/{number}/comments         # inline コメント取得
```
